...
## PHASE 15 — The cross-section (scroll-exploded stack)

The luxury-watch "layers separating on scroll" effect, but the object is Senudi's working model
instead of a watch — so it carries information rather than decorating. CSS 3D transforms driven by
natural scroll progress: no three.js, no pinning, no new dependencies.
**Run this BEFORE Phase 14 — the brutal pass is always the last thing you run.**

```text
CONTEXT: Next.js 14 App Router portfolio, redesign phases 0–12 committed (+ the Work reorder; Phase 13's portrait optional). src/components/Projects.tsx renders the Work section: <section id="work" className="px-5 py-16 sm:px-8 sm:py-24"> → <SectionHeading eyebrow="03 / Work" title="How I've grown" /> → a MotionReveal intro paragraph about ordering → <Timeline> (flagship ProjectCards newest-first, Foundations closing) → PublicationCard. Available systems: framer-motion (useScroll/useTransform/useReducedMotion — the hero aperture in src/components/hero/HeroBridgeBand.tsx is the reference pattern for scroll-linked, NON-pinned progress), Lenis smooth scroll, .terminal-card hairline frames, .marker mono print markers ("+++ … +++"), the FIG chip recipe, green editorial palette (warm ochre / bridge pine / cool slate accents; dark = mint), Newsreader serif + IBM Plex Mono, CursorRing, global grain. HARD RULES from this pack: no pinned or hijacked scrolling anywhere, no glyph-shuffle text effects, transform/opacity only, every effect ships its reduced-motion fallback in the same commit, Lighthouse mobile ≥ 90. Admin (src/app/admin/**), src/lib/**, src/data/content.ts and the zod schema are OFF-LIMITS — this component's layer copy is hardcoded (it is a conceptual diagram, not editable content).

GOAL: Open the Work chapter with an exploded cross-section of how Senudi works — five plates that begin fused as a single slab and separate into a labelled 3D stack as the reader scrolls in. It states her thesis (business ↔ engineering) as an object before the projects prove it.

DO:
1. New component src/components/work/StackCrossSection.tsx ("use client"), mounted in Projects.tsx between <SectionHeading> and the existing intro paragraph (that paragraph stays exactly where it is, directly above <Timeline>).
2. Structure: a stage div with perspective: 1400px and a FIXED height (h-[58svh] sm:h-[66svh], so zero CLS), containing five absolutely-positioned plates centred in it. Each plate: w-full max-w-[34rem], aspect-[16/7], 1px border, translucent fill (bg-surface/70), transform-style: preserve-3d, and a shared base transform of rotateX(58deg) rotateZ(-32deg) — an isometric slab read from above-left.
3. Layers, top → bottom (hardcoded copy; tint = the border + label colour):
   01 THE ASK — "stakeholders, constraints, the actual problem" (warm)
   02 THE TRANSLATION — "user stories, flows, acceptance criteria" (warm)
   03 THE BRIDGE — "the decision that makes both sides work" (bridge — the hero layer: give it a slightly stronger border and a faint bridge-tinted glow)
   04 THE BUILD — "components, endpoints, business logic" (cool)
   05 THE DATA — "schema, queries, what is actually true" (cool)
4. Motion: useScroll({ target: stageRef, offset: ["start 0.85", "end 0.35"] }) — plays as the section passes through the viewport, NEVER pinned. Map progress p (0→1) to each plate's translateY: plate i moves to -(i - 2) * SEP where SEP ≈ 8.5svh (so the middle BRIDGE plate holds position and the others spread symmetrically around it — the bridge is the anchor, which is the point). At p = 0 all plates sit at 0 (one fused slab); by p ≈ 0.75 they are fully separated. Use useTransform with a gentle ease; transform-only, no layout properties.
5. Labels: mono uppercase number + name to the RIGHT of each plate, joined to it by a 1px leader line, with the descriptive line under it in text-muted. Labels fade+slide in staggered (60ms apart) as separation crosses ~40%. They are real text, never baked into anything.
6. Interaction (mirrors the facets cross-wiring): each label is a focusable element; hovering or FOCUSING a label brightens its plate (fill → bg-surface, border → its tint at full strength) and dims the other four with an opacity-only wash; hovering a plate highlights its label. Keyboard parity is mandatory — focus does exactly what hover does. Add a small "+++ THE CROSS-SECTION +++" marker above the stage.
7. Responsive: below sm, flatten the isometry (rotateX ≈ 34deg, rotateZ ≈ -18deg), reduce SEP to ~6svh, and move labels UNDER each plate if they would overflow; if it still feels cramped at 375px, render the flat fallback instead — five stacked hairline bars with their labels, no 3D, no scroll-linking (state which path you took).
8. Reduced motion: render fully separated and static with all labels visible — it becomes a clean labelled architecture diagram. No scroll listener at all in that branch.
9. Accessibility: the stage is an <ol> (or a list with role semantics) so the five layers read as an ordered list to screen readers; decorative leader lines and glows aria-hidden; visible focus ring on each label against the ivory page in both themes.

CONSTRAINTS: No pinning/scroll-hijack; no new dependencies; transform/opacity only; fixed stage height (zero CLS); no content/schema/admin/lib changes; both themes; TypeScript strict; the existing Work intro paragraph, Timeline, Foundations grouping and PublicationCard are untouched.

VERIFY: `npm run build` passes. Scrolling into Work fuses→separates the five plates smoothly with no scroll hijack (the page never feels held); the BRIDGE plate stays anchored while the others spread; labels stagger in and the hover/focus cross-wiring works both directions with keyboard parity; reduced motion shows the diagram fully separated and static; 375px has no horizontal overflow and the layer copy is readable; both themes; Lighthouse mobile still ≥ 90 (this adds five transformed divs — it should cost nothing, confirm). Commit "phase 15: work cross-section".
```

---

## PHASE 16 — The lane: About as a swimming scene

About becomes a full-section cinematic scene — the top-down pool illustration filling it edge to
edge, the swimmer travelling up the lane as the reader scrolls, all About content floating on top.
Mechanically this is Phase 12's contact-scene pattern (which is the pattern that WORKS); the new
part is the scroll-driven swim. **Run before PHASE 14 — the brutal pass is always last.**

```text
CONTEXT: Next.js 14 App Router portfolio. Phases 0–12 committed (ed57331 = "phase 12: full-page contact map scene") plus the Work reorder (50989e5). Phases 13, 14 and 15 are NOT in — About has no portrait plate and public/media/portrait.jpg does not exist — so the element list in DO step 2 is exhaustive; do not go looking for a portrait plate to preserve. (HEAD 0533dc7 is mislabelled "phase 11" but its diff touches only REDESIGN-PROMPTS.md.) About (src/components/About.tsx) currently holds: chapter SectionHeading, an oversized serif pull-quote (first about paragraph), the remaining paragraphs in two columns on lg+, education/additional editorial list rows, and at the end the Facets band (index rail + asymmetric contact-sheet cluster, src/components/Facets.tsx + FacetCard.tsx, driven by src/data/media.ts). PROVEN PATTERN TO COPY — src/components/Contact.tsx from Phase 12: the section carries .plate-dark (a globals.css utility assigning all nine palette variables the .dark block's values, so everything inside renders dark-theme styling in BOTH themes), the section itself carries NO padding (Contact.tsx:64 is exactly `relative w-full overflow-clip plate-dark bg-bg min-h-[100svh]`; the padding lives on the inner content wrapper, Contact.tsx:85), its FIRST child is a sticky frame, and the content wrapper follows as relative z-10 -mt-[100svh]. NOTE: BaseMap is NOT a precedent for scroll-linking — it only uses useInView on its own ref and never reads the section's scroll progress. Media manifest src/data/media.ts holds facets[] including { key: "swim", image: "/media/facet-swim.jpg" } and the unused spare public/media/facet-swim-alt.jpg. framer-motion 12 useScroll/useTransform/useReducedMotion available; Lenis smooth scroll; HARD RULES: no pinned/hijacked scrolling (a sticky backdrop under freely-scrolling content is NOT a pin — content scroll speed is never altered), transform/opacity only, reduced-motion fallback in the same commit, Lighthouse mobile ≥ 90. Admin (src/app/admin/**), src/lib/**, src/data/content.ts, zod schema OFF-LIMITS.

GOAL: The About chapter becomes a lane. The pool illustration fills the whole section edge to edge, the swimmer advances up it as the reader scrolls (with breaststroke rhythm, not conveyor-belt drift), and every existing About element floats above it — legibly.

MEASURED ASSET FACTS (verified — do not re-derive): /media/facet-swim.jpg is 1344×2016 (exactly 2:3). The swimmer is oriented HEAD-DOWN in the source: toes at 31.5% of image height, torso 45–52%, hands 50.8%, white cap 55.8–61.7% (centre 58.7%), kick wake trailing from 26% up to 6%. She is horizontally centred (body axis 51.8%, arm span 31.4%–69.0%). Her full figure spans 30.2% of image height. Both lane ropes sit at the extreme left/right edges (0–8.2% and 93.5–100%).

DO:
1. New component src/components/about/LaneBackdrop.tsx ("use client"), props { sectionRef }: { sectionRef: React.RefObject<HTMLElement> }. THREE nested layers plus frame-sized siblings — do not collapse them into one:
   - FRAME: <div className="sticky top-0 h-[100svh] overflow-hidden" aria-hidden="true">. overflow-hidden HERE is required and safe — the "never overflow-hidden" rule applies to ANCESTORS of a sticky element, not to the sticky element itself. Without it the 190svh media layer paints far below the frame, behind the paragraph columns, with no wash over it.
   - L1 TRAVEL: <motion.div style={{ y: travelY }} className="absolute inset-x-0 top-0 h-[155svh] sm:h-[190svh]"> — scroll travel ONLY, nothing else on this layer. The responsive height is not cosmetic: object-cover is width-limited only when viewportWidth > 1.2667 × viewportHeight, so at 390×844 a 190svh wrapper goes HEIGHT-limited and crops to just 36.5% of the image width, clipping both her hands and both lane ropes. 155svh restores 44.7% at 390×844 and 54.4% at 375×667.
   - L2 STROKE: <motion.div style={{ scale: pulse, rotate: roll, x: sway, transformOrigin: "50% 48%" }} className="absolute -left-[5%] -right-[5%] -top-[2%] -bottom-[2%]"> holding the <img>. The negative insets are mandatory overscan: a 0.5deg roll on a viewport-wide, ~1700px-tall box displaces its corners ~7.5px and the sway adds ~6px, so without overscan a wedge of bare near-black section background flickers in at both edges on every glide. Use INSETS, never a resting scale (a permanent upscale resamples the whole layer for nothing). transformOrigin 50% 48% puts the pivot on her centre of mass (wrapper-y 47.5% desktop / 48.0% mobile) instead of a point up to 45svh off-screen — that difference is what separates "her body rolling" from "the image shearing".
   - THE IMAGE: <img src="/media/facet-swim.jpg" alt="" aria-hidden className="h-full w-full object-cover -scale-y-100" loading="lazy" decoding="async" />. The -scale-y-100 is REQUIRED: the source is head-down, so without the flip she swims feet-first. The image is vertically near-symmetric (straight lane ropes, straight floor stripe) so the flip is invisible. About sits just below the fold — do NOT priority-load; the hero headline must stay LCP.
   - THE SWIM: useScroll({ target: sectionRef, offset: ["start start", "end end"] }) drives travelY from 0 to -55svh below sm / -90svh at sm and up (rule: travel = wrapper height − 100svh). The offset is critical — ["start end","end start"] spans S+100svh of scroll while the frame is only pinned for S−100svh of it, so barely half the travel would be spent on screen and she would rocket in, crawl through the scene, and rocket out (a 5.4× speed inversion). ["start start","end end"] IS the sticky range: she traverses a full 90% of the frame while pinned, entering just below the bottom edge and exiting at the top. Direction check after the flip: at progress 0 her white cap sits roughly mid-frame with her feet BELOW the bottom edge.
   - BREASTSTROKE RHYTHM: do not map progress linearly. Feed useTransform a multi-stop curve of ~8–10 pairs alternating SURGE segments (large travel over small progress — the pull) and GLIDE segments (small travel over large progress — the streamline), about 3 cycles across the PINNED range, strictly monotonic, ending exactly at the travel end. This is the whole difference between "a moving background" and "someone swimming" — tune it until it feels like a stroke.
   - THE STROKE cues, all driven from the SAME surge/glide curve so they land on the pull and release on the glide (this is what makes her SWIM rather than slide):
     (a) Body pulse — L2 scales 1.0 → 1.025 through each surge, easing back over the glide.
     (b) Roll — ±0.5deg on the same beat about the 50%/48% origin (keep under 0.6deg).
     (c) Sway — ±0.4% translateX on the same beat, phase-shifted a quarter cycle behind the pulse.
     (d) Water churn — an aria-hidden radial-gradient div INSIDE L2 (so it rides with her): className="absolute left-1/2 top-[44%] h-[48svh] w-[48svh] sm:h-[60svh] sm:w-[60svh]", opacity 0 → ~0.5 and scale 0.9 → 1.15 on each surge, fading through the glide. CRITICAL: never mix Tailwind translate utilities with framer-motion transforms on the same element — motion's inline transform silently overwrites -translate-x-1/2, landing the churn's top-left corner on the anchor instead of its centre. Centre it INSIDE the style object: style={{ x: "-50%", y: "-50%", opacity: churnO, scale: churnS }}. Measured with the flip: her cap lands at wrapper-y 39.0% (desktop) / 41.3% (mobile) and her hands at 49.0% / 49.2%, so top-[44%] splits the pull region within 1.3pp everywhere. Size MUST be in svh, not % of width — her arm span is 37.6% of wrapper width on desktop but 103% at 390px. Fade the radial to transparent by ~70% so its exact edge never matters.
     Verify the composite reads as swimming, not as a jittering photo — if any single cue is noticeable on its own, it is too strong. All four are disabled under reduced motion. Do NOT hard-code will-change-transform: framer-motion 12 manages it while values animate, and pinning three full-bleed layers costs ~22 MB each at 390×844/DPR3.
   - LEGIBILITY STACK — these are absolute inset-0 SIBLINGS of L1 inside the frame, never children of it (inside a 190svh wrapper a "22%" gradient stop becomes 41.8svh and slides up with the image): a full wash of bg-bg/72 (under .plate-dark, --bg is the near-black value and --ink is the LIGHT one — the wash must be bg-bg, never bg-ink), then a top gradient from-bg to transparent (~22%) and a bottom gradient to-bg (~28%). These anchor text at the scene's edges; they do NOT blend into the neighbouring sections and must not be tuned as if they could — Hero above and Skills below (bg-surface) are ivory in light theme, so About meets its neighbours as a deliberate cut, exactly as Contact already does.
   - REDUCED MOTION: no scroll listener at all; render L1 static at translateY −27.5svh below sm / −45svh at sm and up (half the travel — NOT half the wrapper; −95svh is outside the valid range and would leave an uncovered strip and crop her legs). Verified: those values frame her whole figure with ≥2.3pp margin at 1440×900, 1280×800, 390×844 and 375×667.
   - VIDEO UPGRADE PATH (drop-in later, no redesign): if a real 5s breaststroke loop is ever generated (Higgsfield image-to-video from this exact still, ~7.5 credits), replace the <img> with <video muted loop playsInline autoPlay poster={the still}> at the same object-cover sizing and DELETE cues (a)–(d) — the video supplies the actual stroke while the scroll travel stays. Do not scroll-scrub currentTime (janky on mobile Safari); let it loop freely.
2. About.tsx: add "use client" — it is currently the only SERVER component in its subtree and cannot create the ref the swim needs (all its props are plain serializable data, so the directive is safe). Add const sectionRef = useRef<HTMLElement>(null), put it on the section, and render <LaneBackdrop sectionRef={sectionRef} /> as the first child. The section becomes exactly `relative w-full overflow-clip plate-dark bg-bg min-h-[100svh]` — copy Contact.tsx:64 verbatim. Its current padding (px-5 py-16 sm:px-8 sm:py-24) MUST move off the section onto the inner content wrapper; left on the section it becomes the sticky frame's containing block and the lane renders inset, framed by bare bg-bg — the opposite of edge to edge. Every existing element (SectionHeading, pull-quote, paragraph columns, education/additional rows, Facets band) moves into the relative z-10 -mt-[100svh] wrapper in the SAME order with the same content plumbing; keep its top padding ≥ 5rem at sm+, because overflow-clip newly clips SectionHeading's ghost numeral (absolute -top-14 sm:-top-20).
3. Legibility discipline for the content (the Phase 10 rule): the oversized serif pull-quote may sit on the wash alone; body-size text may not. Give the paragraph columns and the education/additional rows an aria-hidden soft gradient scrim behind them (or a subtle bg-bg/55 panel) so small text always has a floor. Spot-check AA in both themes. There are ZERO Tailwind `dark:` variants anywhere in src (verify with `rg 'dark:' src` — no matches); theming is entirely CSS-variable driven, so About/Facets/FacetCard re-tone cleanly under .plate-dark and need no audit. The real light-theme risk is elements that float OVER the scene from OUTSIDE the subtree and therefore resolve :root's palette — see step 7.
4. Swap the swimmer facet so the same image is not both backdrop and card: in src/data/media.ts point the "swim" facet's image at "/media/facet-swim-alt.jpg" and update its alt to describe the head-on splash. Caption/label unchanged. Eyeball the swap before committing: facet-swim-alt.jpg is the right subject but a very different register from the other three studies — saturated cobalt/orange poster art with light rounded corners baked into the pixels, which read as pale arcs inside the card's square frame. If it fights the set, keep the top-down image on the card and accept the repetition, or re-crop the alt.
5. Facets band on the scene: the four cards are ALREADY opaque (FacetCard's frame is aspect-[2/3] overflow-hidden border border-line with DitherMedia absolute inset-0, whose root carries bg-bg and settles on a full-bleed opaque <img>), and DitherMedia re-tones itself under plate-dark automatically (it reads --ink/--bg off its own wrapper). So add no fill — it would be entirely covered. If the cluster needs to read as plates ON the scene rather than images floating in images, use a visible margin instead: a stronger local wash (bg-bg/85) behind the band, or a hairline outer frame. Do not remove the band.
6. Section rhythm check: Contact is already a full-page dark scene, so the page now reads ivory → DARK lane → ivory skills → ivory work → DARK map. Verify the two dark scenes feel like deliberate bookends rather than repetition; if the About scene weakens Contact's finale, lighten About's wash slightly (bg-bg/60) so Contact stays the darkest, densest moment on the site.
7. ChapterRail (src/components/ChapterRail.tsx) floats over every section (fixed right-6 top-1/2 z-40, mounted in page.tsx outside <main>) and resolves its palette from :root, not from the section it overlaps. Line 54 is `const onScene = active === "contact";` — a single hard-coded id from Phase 12. About becoming a second dark scene reproduces that bug verbatim: in light theme the active label (text-ink, rgb 24 32 26) lands on rgb(17 22 19) at ~1.05:1, i.e. invisible. Generalise it: `const DARK_SCENES = new Set(["about", "contact"]); const onScene = DARK_SCENES.has(active);`. Nothing else in the file changes — its three on-scene branches already hard-code the dark-palette literals.

CONSTRAINTS: No content/schema/admin/lib changes (except the one media.ts image path in step 4); both themes render the identical dark scene; transform/opacity only; zero CLS; LCP stays the hero headline; TypeScript strict. The sticky frame height and the content pull-up MUST use the same unit — h-[100svh] with -mt-[100svh]. Never dvh or vh: svh is fixed against the address-bar-expanded height and cannot jump mid-scroll, dvh can. (Corollary: once the bar collapses the frame ends ~90–100px above the visible bottom — benign only because the bottom gradient resolves to bg-bg, so do not shorten the ~28% bottom stop.)

VERIFY: `npm run build` passes. Scrolling through About makes the swimmer travel UP the lane HEAD-FIRST with a stroke-like surge-and-glide (not a constant slide); the backdrop sticks while content scrolls over it; the page never feels scroll-hijacked. No bare background wedge appears at the frame edges during any glide (the overscan test). All About text is legible in BOTH themes (small text on scrims/panels — check the education rows and facet captions specifically). The chapter rail is readable over the About scene in LIGHT theme — active label, inactive labels, ticks and needle (Phase 12's check, re-run for the second dark scene). The swim facet card shows the head-on splash. Reduced motion: static scene, her whole figure in frame, everything readable. 390×844 and 375×667: no horizontal overflow, her hands and the central floor stripe visible (the lane ROPES are cropped out on every portrait phone at any wrapper height that still yields useful travel — that is expected, the lane must read from the floor stripe alone; do not chase the ropes on mobile). Measure About's rendered height — the swim only plays over (height − 100svh), so it needs ≥ ~250svh; it currently measures ~270svh desktop / ~400svh mobile. Lighthouse mobile ≥ 90 and LCP still the hero headline. Commit "phase 16: about as a swimming lane".
```

---

## PHASE 17 — Work timeline as a tracked spine (sui.io treatment)

Restyle the EXISTING Work timeline into the sui.io scroll-tracked look: a dotted vertical spine, a
marker that travels down it as you scroll with the spine lit above / dim below, numbered square
nodes wired to each card, and the card the marker is passing lighting up. Pure CSS + the scroll
progress the timeline already computes — no new dependencies, no new section.

```text
CONTEXT: Next.js 14 App Router portfolio, redesign phases 0–12 + Work reorder committed (Phase 16 "swimming lane" may also be committed but is unrelated — it only touches About; leave it). The Work section is src/components/Projects.tsx → src/components/work/Timeline.tsx. CURRENT Timeline.tsx (read it, these are the real facts): a "use client" <ol ref className="relative mt-12 space-y-6 pl-8 sm:pl-10">; a hairline spine <span className="absolute left-0 top-0 h-full w-px bg-line" />; a scroll-progress line — motion.span with style={{ scaleY }} className="absolute left-0 top-0 h-full w-px origin-top bg-ink", where scaleY = useSpring(scrollYProgress, {stiffness:140,damping:28,mass:0.5}) and scrollYProgress = useScroll({ target: ol, offset: ["start 0.75","end 0.5"] }); a staticSpine boolean (set on prefers-reduced-motion) that swaps the animated line for a full bg-ink line. TimelineItem renders <li className="relative"> with a dot: motion.span className={`absolute top-7 -left-[38px] h-3 w-3 rounded-full ring-4 ring-bg sm:-left-[46px] ${dotClass}`} that scales/fades in whileInView (once), wrapping its children in <MotionReveal>. Projects.tsx maps the flagship ProjectCards (newest-first, admin-controlled order) then a Foundations TimelineItem LAST (dotClass "bg-muted" — the origin), then a PublicationCard AFTER the <Timeline>; dotClass colors come from dotPalette = ["bg-bridge","bg-bridge","bg-cool","bg-warm"]. ProjectCard headers currently show timeframe (mono uppercase) then an UPPERCASE serif title. Green editorial palette, mono labels, .terminal-card, "+++" markers already established. framer-motion useScroll/useTransform/useSpring/useInView available. Admin (src/app/admin/**), src/lib/**, src/data/content.ts, zod schema OFF-LIMITS; the ".NET foundation" grouping and the admin order must not change. Apply intent if exact classes differ.

GOAL: The Work timeline reads like sui.io's tracked index — a dotted spine with a marker that travels as you scroll (lit above, dim below), numbered square nodes elbow-connected to each card, and the passed card highlighted — while keeping the serif ProjectCards, Foundations origin, and publication block intact.

DO:
0. COORDINATE-FRAME COHERENCE (the thing that makes or breaks this — read first): the marker, the lit/dim boundary, and the active card must ALL derive from ONE scroll value or they point at different items. The current code springs scaleY separately (useSpring(scrollYProgress,{stiffness:140,damping:28,mass:0.5})) — DELETE that separate spring. Create exactly one MotionValue `p` (raw scrollYProgress, or a single useSpring of it — decide once; raw tracks scroll 1:1 and agrees best with the active card, a shared spring is smoother but lags during fast flicks) and feed the fill, the marker, AND the active-index detection all from `p`. Note the existing useScroll offset ["start 0.75","end 0.5"] makes `p` a fraction of the LIST, so the marker rides ~50–75% of the viewport, NOT screen center — which is exactly why active detection must come from `p` (step 4), not a viewport-center observer.
1. Spine → dotted, in Timeline.tsx (two byte-identical-box spans so their dot patterns phase-lock):
   - DIM track: a 1px span, absolute top-0 left-0 h-full, aria-hidden, NO border/padding, background repeating-linear-gradient(to bottom, rgb(var(--line)) 0 2px, transparent 2px 8px), with background-origin:border-box; background-position:0 0.
   - LIT overlay: a second span with an IDENTICAL box (absolute top-0 left-0 w-px h-full, NO border/padding — any border/padding shifts its dots to sit BETWEEN the dim dots and you get a seam at the marker), same gradient in rgb(var(--ink)) (or bridge — step 4), and reveal it with clip-path: inset(0 0 calc((1 - p) * 100%) 0) driven from `p` (useMotionTemplate). Use clip-path, NOT height and NOT scaleY: scaleY stretches the dot pitch; animating height triggers layout every frame (violates the no-thrash rule); clip-path is composited, keeps the box geometry fixed so the dot phase can never drift, and its bottom edge IS the marker boundary.
2. Traveling marker (the sui.io square): a ~8px solid square (bg-ink or bridge, ring-4 ring-bg so it reads on the dots), aria-hidden, rendered as a DIRECT child of the <ol> (so top:% resolves against the same ol padding-box as the fill — inside an <li> it resolves against the li and mis-scales completely). style={{ top }} = useTransform(p,[0,1],["0%","100%"]) with translate(-50%,-50%) in the STYLE object (both axes — X centers the 8px square on the 1px spine at left:0, Y centers it on the boundary; a Tailwind -translate class would be overwritten by framer's inline transform). Because both the marker top and the clip-path bottom come from `p`, the marker rides the lit/dim boundary exactly. Not rendered at all under reduced motion (step 6).
3. Numbered nodes + card numbers (the gutter is only 32px — a legible number does NOT fit in it, so the number's home is the card header at ALL breakpoints; measured: node+ring occupy ~x10–x30 straddling the spine, leaving ~22px clear before the card — room for the square node plus a short elbow tick and nothing more):
   - Per-item node: the round dot becomes a small SQUARE (keep ring-4 ring-bg + the dotClass color) at the existing top-7 -left-[38px] sm:-left-[46px] offsets, plus a 1px ELBOW tick (≤12px) from the node into the card. No number in the gutter.
   - Card number: echo the 1-based index into the ProjectCard header — but the header ALREADY opens with a muted mono uppercase timeframe line above the serif title, so do NOT add a second muted mono line (it would read as redundant stacked mono). MERGE the number into that line as one mono row, e.g. "01 — {project.timeframe}", with the "01" token in text-ink (slightly stronger) while the timeframe stays text-muted; the "01" is the resting label and brightens/holds ink when active. Keep the serif title exactly as is.
   - Thread the index from Projects.tsx: flagships.map gives i+1; the Foundations TimelineItem (a separate item after the map) takes flagships.length+1 — OR render Foundations' node as a distinct hollow "origin" square with no number (state which). Pass `index` and `active` (step 4) into TimelineItem/ProjectCard.
4. Active state derived from the SAME `p` (NOT a viewport-center IntersectionObserver — per step 0 the marker is list-proportional and does not sit at screen center, so a center band would light a different card than the marker is passing): lift `activeIndex` state into Timeline; measure each <li>'s offsetTop+height within the <ol> on mount and on resize (ResizeObserver); on `p` change (useMotionValueEvent) set active = the item whose span contains marker_y = p × olHeight. Exactly one active by construction; none only at the very ends. Active look: node fills to its dotClass color (inactive nodes hollow/dim), the "01" brightens to text-ink, the card border border-line → border-ink (or bridge tint), ~250ms. (If an observer is unavoidable, mirror the SINGLE-observer pattern already in Nav.tsx / ChapterRail.tsx — one observer over all nodes, rootMargin "-45% 0px -50% 0px", active = last intersecting — never one-observer-per-item, which double-lights ~57px at every boundary. But the `p`-derived route is preferred because it agrees with the marker.) Legibility never depends on active state — all cards read fully at rest.
5. Preserve everything else: the Foundations card stays LAST as the origin node; the PublicationCard stays after the timeline untouched; the ".NET foundation" grouping and admin order are untouched; the flagship count is dynamic (number from the map index, do not hardcode).
6. Reduced motion (make "no scroll listener" actually true and kill the spine-pop flash): hooks can't be conditional, so extract the marker + lit overlay + every useScroll/useSpring/useTransform/useMotionValueEvent consumer into a CHILD component that Timeline renders ONLY when motion is allowed — under reduced motion that child never mounts, so framer attaches no scroll/resize listener. Render the static lit dotted spine from a CSS @media (prefers-reduced-motion: reduce) rule or an SSR-safe initial state, NOT from post-mount useEffect state — the current staticSpine-via-useEffect paints one frame of the empty (clip 0%) overlay before swapping, a visible pop. All cards render resting (no active highlighting) under reduced motion.

CONSTRAINTS: No content/schema/admin/lib changes; the grouping + order logic untouched; transform/opacity/background-position/height animations only (no layout thrash); both themes (dotted spine, marker, and active states must read on ivory AND dark); TypeScript strict; no new dependencies; respect prefers-reduced-motion (static lit spine, no marker, resting cards).

VERIFY: `npm run build` passes. Scrolling Work: the marker travels down the dotted spine and sits EXACTLY on the lit/dim boundary even during a fast flick (they share one `p`); the lit dots land on the dim dots with NO seam; each card lights up (border + "01" + node) as the marker reaches it and releases as it passes, with EXACTLY ONE card active at a time (no double-light at card boundaries). Numbers read 01…0N in display order with Foundations closing as the origin node; the number lives in the card header, not the gutter; the publication block is unchanged; the drag-strip inside Foundations still works. 375px: the node + elbow sit in the 32px gutter with no horizontal overflow. Reduced motion: static lit dotted spine, no marker, all cards resting and readable — and NO scroll listener attached (the motion child never mounts) and no spine-pop flash on load. Both themes. Commit "phase 17: tracked timeline spine".
```
