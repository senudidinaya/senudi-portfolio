...
// The GL scene, dynamically imported by its call sites with ssr:false. This
// file (and particleImageSampler/particleShaders, which it alone imports) is
...
import type { ParticleImageProps } from "./particleTypes";
import { samplePointField, type SampledField } from "./particleImageSampler";
...
export function ParticleImage({ onContextLost, ...scene }: ParticleImageProps) {
...
function Scene({
  ...
  repelScale = 1,
  edgeFade = 0,
  pointerSource = "canvas",
  blend = "auto",
  grade = null,
  onReady,
}: Omit<ParticleImageProps, "onContextLost">) {
    ...
    () =>
      new THREE.ShaderMaterial({
        uniforms: {
          ...
          uOpacity: { value: 1 },
          uAmp: { value: dispersalAmp },
          uRepel: { value: repelScale },
  ...
  // must stay NormalBlending, only dark gets the additive glow. blend
  // "normal" opts out entirely, for a subject (a portrait) whose own colour
  // is the point and would blow out if accumulated.
  useEffect(() => {
    material.blending =
      blend === "normal" || theme !== "dark" ? THREE.NormalBlending : THREE.AdditiveBlending;
  ...
  }, [theme, blend, material]);

  useEffect(() => {
    material.uniforms.uAmp.value = dispersalAmp;
    material.uniforms.uRepel.value = repelScale;
    material.uniforms.uEdgeFade.value = edgeFade;
  }, [dispersalAmp, repelScale, edgeFade, material]);

  // rounded key so sub-pixel resize jitter doesn't re-trigger a full resample.
  // budget is read off the tier here so the dep is a number — an inline
  // pointBudget object literal would otherwise resample on every render.
  const sizeKey = `${Math.round(size.width)}x${Math.round(size.height)}`;
  const budget = pointBudget[tier];
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [sizeKey, budget, src, grade, planeScale, anchorY]);
  ...
  useEffect(() => {
    ...
    geo.setAttribute("aAlpha", new THREE.BufferAttribute(sample.alpha, 1));
  ...
  // fine-pointer only; NDC read from a LIVE rect each move (never a cached
  // rect — the parallax translate + object-cover overflow sit between
  // screen-px and plane-space, so only the live canvas rect is guaranteed
  // correct)
  useEffect(() => {
    const canvas = gl.domElement;
    ...
    // a pointer-events:none canvas never sees a pointer event of its own, so
    // it listens on the window and keeps deriving NDC from its own live rect
    // — the same maths, just a source that isn't gated by hit-testing
    const target: EventTarget = pointerSource === "window" ? window : canvas;
    const leaveTarget: EventTarget =
      pointerSource === "window" ? document.documentElement : canvas;
    target.addEventListener("pointermove", handleMove as EventListener, { passive: true });
    leaveTarget.addEventListener("pointerleave", handleLeave, { passive: true });
    return () => {
  ...
  useFrame((_, delta) => {
    ...
    material.uniforms.uScroll.value = dissolve.get();
